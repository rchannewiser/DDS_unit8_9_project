...
# Read in the Data
```{r Data}
Beer <- read.csv("~/Documents/MSDS_6306_Doing-Data-Science/Unit 8 and 9 Case Study 1/Beers.csv")
Breweries <- read.csv("~/Documents/MSDS_6306_Doing-Data-Science/Unit 8 and 9 Case Study 1/Breweries.csv")
```
...
iterations = 100 # This is the amount of times to test each k 
...
## Let's Analyze effects of Bitterness and Alchol content on Lager style.
```{r Ounces Barchart, out.width = "2400px"}

BeerCleanMeanIBU %>% 
  mutate(classif = ifelse(str_detect(str_to_lower(Style),"ipa"),"IPA",ifelse(str_detect(str_to_lower(Style),"ale"),"Ale",ifelse(str_detect(str_to_lower(Style),"lager"),"Lager","Others")))) %>% 
  filter(classif != 'Others')  %>% 
  ggplot(aes(x=ABV, y = IBU, fill=classif)) + geom_point(aes(colour=classif)) + geom_smooth() + ggtitle("IBU VS ABV by Styles(Ale, Lager, IPA)")
